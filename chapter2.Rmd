...
Multiple R-squared tells how well the variables explain the variation of the target. In this case only 20.48 % of variation of exam points is explained by `attitude` and `stra`.


We can produce the following diagnostic plots: Residuals vs Fitted values, Normal QQ-plot and Residuals vs Leverage. 

```{r}
# Choose the plots 1, 2 and 5
plot(m2, which = c(1))
plot(m2, which = c(2))
plot(m2, which = c(5))
```
