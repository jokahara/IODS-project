...
There significant difference over time, but not with different treatments.
...
We will use the lme4 package which offers efficient tools for fitting linear and generalized linear mixed-effects models. 
...
```{r}
fit <- lmer(bprs ~ week + treatment + (week | subject), data = BPRS, REML = FALSE)
summary(fit)
```
